...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="horse2zebra",
                        help="dataset name. (default:`horse2zebra`)"
                             ...
                             "cezanne2photo, ukiyoe2photo, vangogh2photo, facades, "
                             "]")
    ...
    parser.add_argument("-b", "--batch-size", default=1, type=int,
                        metavar="N",
                        help="mini-batch size (default: 1), this is the  "
                             "batch size of a GPU on the current node when "
                             "using Data Parallel or Distributed Data Parallel")
    parser.add_argument("--lr", type=float, default=0.0002,
                        help="learning rate. (default:0.0002)")
    parser.add_argument("-p", "--print-freq", default=100, type=int,
                        metavar="N", help="print frequency. (default:100)")
    parser.add_argument("--save_interval", type=int, default=1, help="save checkpoint frequency")
...
def main():
    ...
    if args.cuda:
        ...
        print("{} GPUs is/are available".format(n_gpu))
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size*n_gpu, shuffle=True, pin_memory=True)
    ...
    if n_gpu > 1:
        netG_A2B = torch.nn.DataParallel(netG_A2B, device_ids=range(n_gpu))
        netG_B2A = torch.nn.DataParallel(netG_B2A, device_ids=range(n_gpu))
        netD_A = torch.nn.DataParallel(netG_B2A, device_ids=range(n_gpu))
        netD_B = torch.nn.DataParallel(netG_B2A, device_ids=range(n_gpu))
    """
    ...
    logdir = os.path.join("./runs", args.dataset)
    ...
    for epoch in range(0, args.epochs):
        ...
        for i, data in progress_bar:
            ...
            loss_D.update((errD_A + errD_B).item(), batch_size)
            loss_G.update(errG.item(), batch_size)
            ...
            if i % args.print_freq == 0:
                if batch_size > 1:
                    real_image_A = real_image_A[0].unsqueeze(0)
                    real_image_B = real_image_B[0].unsqueeze(0)
        ...
        # tensorboard 
        writer.add_scalar('train/loss_D', loss_D.avg, epoch)
        writer.add_scalar('train/loss_G', loss_G.avg, epoch)
